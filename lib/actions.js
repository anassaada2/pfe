...
// Initialisation de Pinata
const pinata = new PinataSDK({
  pinataJwt: process.env.PINATA_JWT,
  pinataGateway: process.env.NEXT_PUBLIC_GATEWAY_URL,
});

// Fonction pour extraire le CID de l'URL
...
export const deleteCalculateur = async (formData) => {
  try {
    ...
    // Trouver le calculateur pour obtenir l'URL avant suppression
    ...
    // Supprimer le fichier de Pinata si une URL existe
    if (calculateur.url) {
      try {
        ...
        console.error(
          "Erreur lors de la suppression du fichier Pinata:",
          pinataError
        );
    ...
    // Supprimer le calculateur de la base de données
...
export const deletePinataFile = async (formData) => {
  try {
    ...
    // Mettre à jour le calculateur pour supprimer l'URL
    const updated = await Calculateur.findByIdAndUpdate(
      id,
      { url: null },
      { new: true }
